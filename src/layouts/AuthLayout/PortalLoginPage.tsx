  ...
  return (
    <AuthLayout>
      <div className="overflow-hidden rounded-2xl border border-line bg-surface shadow-xl">
        ...
        <form
          onSubmit={(e) => { e.preventDefault(); setLoading(true); setTimeout(() => navigate(redirectTo), 400); }}
          className="space-y-4 p-8"
        >
          <FormField label={emailLabel}><Input icon={Mail} defaultValue="demo@techxserve.co" /></FormField>
          <FormField label="Password"><Input type="password" icon={Lock} defaultValue="demo" /></FormField>
